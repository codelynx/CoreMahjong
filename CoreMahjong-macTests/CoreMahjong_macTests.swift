...
class CoreMahjong_macTests: XCTestCase {
	...
	func test和了役Set() {
		do {
			// 二盃口は一盃口に優先する
			let 和了役列: [和了役型] = [.二盃口, .一盃口]
			let 和了役群 = Set(役列: 和了役列, 上位下位役一覧: 上位下位役一覧)
			XCTAssert(和了役群 == Set([.二盃口]))
		}
		do {
			// 現実の麻雀ではあり得ない
			let 和了役列: [和了役型] = [.二盃口, .一盃口, .混一色, .清一色, .混全帯么九, .純全帯么九, .混全帯么九, .混老頭]
			let 和了役群 = Set(役列: 和了役列, 上位下位役一覧: 上位下位役一覧)
		...
		do {
			// 下位役満
			let 和了役列: [和了役型] = [.四暗刻]
			let 和了役群 = Set(役列: 和了役列, 上位下位役一覧: 上位下位役満一覧)
		...
		do {
			// 四暗刻単騎は四暗刻に優先する
			let 和了役列: [和了役型] = [.四暗刻単騎, .四暗刻]
			let 和了役群 = Set(役列: 和了役列, 上位下位役一覧: 上位下位役満一覧)
			XCTAssert(和了役群 == Set([.四暗刻単騎]))
		}
		do {
			// 現実の麻雀ではあり得ない
			let 和了役列: [和了役型] = [.国士無双十三面張, .国士無双, .大四喜, .小四喜, .發なし緑一色, .緑一色]
			let 和了役群 = Set(役列: 和了役列, 上位下位役一覧: 上位下位役満一覧)
	...
	func test複合役判定() {
		//	🀇, 🀈, 🀉, 🀊, 🀋, 🀌, 🀍, 🀎, 🀏, 🀙, 🀚, 🀛, 🀜, 🀝, 🀞, 🀟, 🀠, 🀡, 🀐, 🀑, 🀒, 🀓, 🀔, 🀕, 🀖, 🀗, 🀘
		//	🀆, 🀅, 🀄︎, 🀀, 🀁, 🀂, 🀃
		XCTAssertTrue(和了判定(手牌列: "🀇🀏🀙🀡🀐🀘🀆🀅🀄︎🀀🀁🀂🀃".牌列, 副露面子列: [], 和了牌: 牌識別子型.🀏.牌.栄和) == Set([.国士無双十三面張]))
		XCTAssertTrue(和了判定(手牌列: "🀇🀈🀉🀊🀋🀌🀍🀎🀜🀝🀞🀁🀁".牌列, 副露面子列: [], 和了牌: 牌識別子型.🀏.牌.栄和) == Set([.一気通貫]))
	}
